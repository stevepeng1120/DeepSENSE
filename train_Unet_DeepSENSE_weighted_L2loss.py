...
def image_augmentation3D(X, Y, mask, rho):
  ...
  Returns:
    ...
    mask
  """
  concat_image = tf.concat([X, Y, rho], axis=-1)
  # XY rotation
...
def get_X_Y_mask(queue, augmentation=True):
  ...
  X_path, Y_path, mask_path, rho_path = tf.decode_csv(csv_content, record_defaults=[[""], [""], [""], [""]) # X, Y, mask, rho
  print "Finish decode csv_content"
  
  X_bytes = tf.read_file(X_path)
  Y_bytes = tf.read_file(Y_path)
  mask_bytes = tf.read_file(mask_path)
  rho_bytes = tf.read_file(rho_path)
  ...
  rho = tf.decode_raw(rho_bytes, np.float32)
  rho = tf.reshape(rho, [2, 36, 159, 159])
  rho = tf.cast(rho, tf.float32)# cast to another type
  rho = tf.transpose(rho,perm=[3,2,1,0])
  rho = tf.reshape(rho, [159, 159, 36, 2])
  ...
  if augmentation:
	X, Y, mask, rho = image_augmentation3D(X, Y, mask, rho)

  print "Finish loading dataset!!"
  return X, Y, mask, rho, Y_path
...
def rho_weighted_loss(x, y, rho):
  _, H, W, Z, _ = y.get_shape().as_list()[0:]
  x_ = tf.complex(x[:,:,:,:,0:16], x[:,:,:,:,16:32])
  y_ = tf.complex(y[:,:,:,:,0:16], y[:,:,:,:,16:32])
  rho_ = tf.complex(rho[:,:,:,:,0:1], rho[:,:,:,:,1:2])

  rho2 = tf.reshape(rho_, [-1, H, W, Z, 1])
  rho2 = tf.concat([rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2, rho2], axis=-1);

  x_multi_coil = tf.multiply(x_, rho2)#
  y_multi_coil = tf.multiply(y_, rho2)

  sub = tf.subtract(x_multi_coil, y_multi_coil) # image weighted L2 loss
  sub = tf.concat([tf.real(sub), tf.imag(sub)], axis=-1);
  weighted_loss = tf.nn.l2_loss(sub)
  return weighted_loss
  
def make_train_op(y_pred, y_true, rho):
  ...
  """
  loss1 = rho_weighted_loss(y_pred, y_true, rho)
...
def main(flags):
  ...
  rho = tf.placeholder(tf.float32, shape=[None, 159, 159, 36, 2], name="rho")
  ...
  tf.add_to_collection("inputs", rho)
  tf.add_to_collection("outputs", pred)

  update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)

  with tf.control_dependencies(update_ops):
    train_op = make_train_op(pred, y, rho)
  ...
  weighted_error_op = rho_weighted_loss(pred, y, rho)
  weighted_error_op = tf.Print(weighted_error_op, [weighted_error_op])
  tf.summary.scalar("Weighted_error", weighted_error_op)

  train_csv = tf.train.string_input_producer([train_file_name])
  test_csv = tf.train.string_input_producer([test_file_name])
 
  train_X, train_Y, train_mask, train_rho, train_path = get_X_Y_mask(train_csv, augmentation=True)
  test_X, test_Y , test_mask, test_rho, test_path = get_X_Y_mask(test_csv, augmentation=False)

  X_batch_op,  y_batch_op, rho_batch_op, mask_batch_op, path_batch_op = tf.train.shuffle_batch([train_X, train_Y, train_rho, train_mask, train_path],
                          batch_size=flags.batch_size,
                          capacity=flags.batch_size * 30,
                          min_after_dequeue=flags.batch_size*5,
                          allow_smaller_final_batch=True)

  X_test_op, y_test_op, rho_test_op, mask_test_op, path_test_op = tf.train.batch([test_X, test_Y, test_rho, test_mask, test_path],
                      batch_size=flags.batch_size,
      ...
      for epoch in range(flags.epochs):
        if flags.test == True: # or epoch % 100 == 99: # testing while training
          ...
          for num_b in range(0, n_test+1, flags.batch_size):
			...
			X_batch, y_batch, rho_batch, mask_batch, path_batch = sess.run([X_test_op, y_test_op, rho_test_op, mask_test_op, path_test_op])
            ...
            rho_batch = np.reshape(rho_batch, (-1,159,159,36,2))
          ...
          for num_b in range(0, n_train, flags.batch_size):
            X_batch,  y_batch, rho_batch, mask_batch, path_batch = sess.run([X_batch_op, y_batch_op, rho_batch_op, mask_batch_op, path_batch_op])
            X_batch = np.reshape(X_batch, (-1,159,159,36,32))
            y_batch = np.reshape(y_batch, (-1,159,159,36,32))
            rho_batch = np.reshape(rho_batch, (-1,159,159,36,2))
            mask_batch = np.reshape(mask_batch, (-1,159,159,36,1))
            _, step_weighted_err, step_summary, global_step_value = sess.run([train_op, weighted_error_op, summary_op, global_step],
            feed_dict={X: X_batch, 
                      y: y_batch, 
                      mask_: mask_batch,
                      rho: rho_batch,
